...
////////execl file handling 
	...
	public function import_file() {
		...
		/*
		//// Upload Code
		do_action('my_admin_notice');
		// Hook to Chanage the Upload path
		add_filter('upload_dir','uploaddir');	
		function uploaddir( $param ){
			$mydir = 'upload';
		
			$param['path'] = plugin_dir_path( __FILE__ ) . $mydir;
			$param['url'] = plugin_dir_path( __FILE__ ) . $mydir;
		...
		$files = glob( plugin_dir_path( __FILE__ )."upload/*" );
		if ($files){
			array_map('unlink', glob( plugin_dir_path( __FILE__ )."upload/*"));
		}  
		
		$uploadedfile = $_FILES[$this->prefix . '_import_file'];
		$upload_overrides = array( 'test_form' => false );
		$inputFileName = wp_handle_upload( $uploadedfile, $upload_overrides );
		*/
