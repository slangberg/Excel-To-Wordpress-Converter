...
public $remove_entire_tag = array ('comment','script','style','h4');//tags whose content will also  be removed -- (will be editble from plugin later)
	...
	foreach($html->find('ul') as $e)
{
		$nxt =  $e->next_sibling ();	
		if($nxt->tag == "ul"){
			$e->outertext = "<div class='wrap'>"  . $e->outertext;
			$nxt->outertext = $nxt->outertext . "</div>";
		}
										...
										if ($added_page!=NULL){
											...
											global $wpdb; $parent_postID;
											 if ($this->legacy_page_options['es_parentPage'] != ""){ // Get Parent URL path: after-hours-and-weekend-appointments	
											    ...
											    $url = parse_url($value[$this->legacy_page_options['es_parentPage']]);
								   				$url_Slug = ltrim($url['path'],'/');/// strips the first slash off path grneated by php
								   				
												echo "Post Url -------------<br>";
												echo "parent url: " . $url_Slug;
												echo "<br>------------- <br>";
												
												
												
												
												$parent_postID = $wpdb->get_var("SELECT post_id FROM $wpdb->postmeta WHERE meta_value='$url_Slug'" );
...
public	function clean_post_html($content,$old_post_id,$oldurl) {//usehtml parser to clean html
			...
			$html=$this->special_html_rules_after($html);
